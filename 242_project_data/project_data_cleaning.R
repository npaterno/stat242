...
file_path <- here::here("242_project_data//")
...
### Cell Phone Data ###
clean_cell_2 <- cell_phones_total %>% 
...
test_data <- clean_cell %>% 
  full_join(clean_life_exp) %>% 
  left_join(clean_net_use) %>% 
  left_join(clean_population)
